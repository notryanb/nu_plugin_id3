            ...
            match tag {
                Ok(tag) =>  {                            
                    ...
                    dict.insert_untagged(
                        "title",
                        tag.title().map_or_else(|| UntaggedValue::nothing(), |title| UntaggedValue::string(title))
                    );

                    dict.insert_untagged(
                        "album",
                        tag.album().map_or_else(|| UntaggedValue::nothing(), |album| UntaggedValue::string(album))
                    );
                    
                    dict.insert_untagged(
                        "artist",
                        tag.artist().map_or_else(|| UntaggedValue::nothing(), |artist| UntaggedValue::string(artist))
                    );

                    dict.insert_untagged(
                        "year",
                        tag.year().map_or_else(|| UntaggedValue::nothing(), |year| UntaggedValue::int(year))
                    );

                    dict.insert_untagged(
                        "track number",
                        tag.track().map_or_else(|| UntaggedValue::nothing(), |track| UntaggedValue::int(track))
                    );
                    ...
                    dict.insert_untagged(
                        "genre",
                        tag.genre().map_or_else(|| UntaggedValue::nothing(), |genre| UntaggedValue::string(genre))
                    );

                    dict.insert_untagged(
                        "disc",
                        tag.disc().map_or_else(|| UntaggedValue::nothing(), |disc| UntaggedValue::int(disc))
                    );
